...
- Model admission ignores HTTP `Content-Length` because static hosts can report the compressed transfer size. It validates the decoded byte array's exact length and SHA-256, so truncated or substituted model content still fails closed.
